...
export default function AccountPage() {
  ...
  const handleUpdateFund = () => {
    ...
    const result = setInitialMonthlyFund(year, month, amount, 'Manual fund update');
    
    if (result) {
      setNewFund('');
      loadData();
      setSuccess(`Fund of ${amount.toLocaleString()} MMK set successfully`);
  ...
  const handleAddFund = () => {
    ...
    if (result) {
      ...
      setError('Failed to add fund. Set initial fund first.');
    }
  };
    ...
    <main className="min-h-screen pb-20" style={{ background: '#090b0d', color: '#fff' }}>
      ...
      <div className="w-full max-w-md mx-auto px-2 pt-6 space-y-6">
        ...
        <Card className="p-6 border" style={{ background: 'linear-gradient(135deg, #1a3a1a 0%, #0a1a0a 100%)', borderColor: '#00c853', borderBottomWidth: '4px' }}>
          ...
          <div className="text-4xl font-black" style={{ color: '#00c853' }}>{wallet.toLocaleString()}</div>
        ...
        <Card className="p-6 border" style={{ background: 'linear-gradient(135deg, #1e2329, #111)', borderColor: '#ffc107', borderBottomWidth: '4px' }}>
          <div className="flex items-center justify-between mb-2">
            ...
            {currentMonthlyFund.hasFund && <div className="text-[10px] text-green-500">✓ Ready</div>}
          </div>
          <div className="text-4xl font-black" style={{ color: '#ffc107' }}>{currentMonthlyFund.total.toLocaleString()}</div>
          <div className="text-xs text-gray-500 mt-2">10,000 - 200,000 MMK • No 30-day limit</div>
          {currentMonthlyFund.hasFund && currentMonthlyFund.initial !== currentMonthlyFund.total && (
            <div className="text-[10px] text-gray-500 mt-1">(Initial: {currentMonthlyFund.initial.toLocaleString()} MMK + Additions)</div>
          )}
        </Card>

        {/* Update Fund Section */}
        <Card className="p-6 border" style={{ background: '#1e2329', borderColor: '#222' }}>
          <h3 className="font-bold mb-4">Set / Update Fund</h3>
          ...
          <div className="p-3 rounded-lg mb-4 text-sm" style={{ background: 'rgba(0, 200, 83, 0.1)', borderLeft: '3px solid #00c853', color: '#00c853' }}>
            ✅ No 30-day limit - edit anytime
          </div>
          ...
          <Button onClick={handleUpdateFund} disabled={!newFund} className="w-full py-6 font-bold text-black disabled:opacity-50" style={{ background: '#ffc107' }}>Set Fund</Button>
        </Card>
          ...
          <Card className="p-6 border" style={{ background: '#1e2329', borderColor: '#222' }}>
            ...
            {!showAddForm ? (
              <Button onClick={() => setShowAddForm(true)} className="w-full py-3 text-sm" style={{ background: '#2a2a2a', color: '#ffc107', border: '1px solid #ffc107' }}>
                <Plus size={14} className="mr-1" /> Add Additional Fund
              </Button>
            ) : (
              <div>
                ...
                <Textarea value={additionNote} onChange={(e) => setAdditionNote(e.target.value)} placeholder="Reason for adding fund" className="mb-2 resize-none" rows={2} style={{ background: '#000', borderColor: '#444', color: '#fff', fontSize: '12px' }} />
                <div className="flex gap-2">
                  <Button onClick={() => setShowAddForm(false)} className="flex-1 py-2 text-sm" style={{ background: '#333', color: '#fff' }}>Cancel</Button>
                  <Button onClick={handleAddFund} disabled={!additionAmount || !additionNote.trim()} className="flex-1 py-2 text-sm" style={{ background: '#ffc107', color: '#000' }}>Add Fund</Button>
                </div>
              </div>
            )}
          </Card>
        ...
        <Card className="p-6 border" style={{ background: '#1e2329', borderColor: '#222' }}>
          ...
          {showHistory && (
            <div className="space-y-3 max-h-64 overflow-y-auto">
              {fundHistory.length === 0 && <div className="text-center text-xs text-gray-500 py-4">No fund history yet.</div>}
              {fundHistory.map((history) => (
                <div key={`${history.year}-${history.month}`} className="p-3 rounded-lg" style={{ background: history.year === year && history.month === month ? '#ffc10720' : '#0f1419', border: '1px solid #333' }}>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-xs font-medium" style={{ color: '#ffc107' }}>{history.monthName}</span>
                    <span className="text-xs text-white">Total: {history.totalFund.toLocaleString()} MMK</span>
                  </div>
                    ...
                    {history.history.map((entry: any) => (
                      <div key={entry.id} className="text-[10px] flex justify-between items-center py-1 border-t border-gray-800">
                        <div className="flex items-center gap-2">
                          ...
                          <span style={{ color: entry.type === 'add' ? '#00c853' : '#ffc107' }}>{entry.newAmount.toLocaleString()} MMK</span>
                          {entry.previousAmount && <span className="text-gray-500">(was {entry.previousAmount.toLocaleString()} MMK)</span>}
                        </div>
        ...
        <Card className="p-4 border text-xs" style={{ background: '#0f1215', borderColor: '#222' }}>
          <div className="space-y-2" style={{ color: '#888' }}>
            ...
            <p>• Maximum fund: 200,000 MMK</p>
            <p>• Minimum fund: 10,000 MMK</p>
            <p>• All changes are tracked with history</p>
